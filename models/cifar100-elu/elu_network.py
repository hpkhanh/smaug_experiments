...
def create_elu_model():
  with Graph(name="elu_smv", backend="SMV", mem_policy=AllDma) as graph:
    # Tensors and kernels are initialized as NCHW layout.
    input_tensor = Tensor(
        data_layout=NHWC, tensor_data=generate_random_data((1, 32, 32, 3)))
    conv0_stack0_tensor = Tensor(
        data_layout=NHWC, tensor_data=generate_random_data((192, 5, 5, 3)))
